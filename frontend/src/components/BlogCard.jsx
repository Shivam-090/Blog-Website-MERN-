...
const BlogCard = ({blog}) => {
  const {title, description, category, image, date, _id} = blog;
  const navigate = useNavigate();  
  return (
    <div onClick={()=> navigate(`/blog/${_id}`)} className='w-full rounded-lg overflow-hidden shadow hover:scale-102 hover:shadiow-primary/25 duration-300 cursor-pointer'>
